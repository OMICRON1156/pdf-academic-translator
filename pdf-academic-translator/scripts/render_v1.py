...
FONT_BODY_FILE = os.path.join(BASE, "assets", "NotoSerifSC-Regular.ttf")
FONT_HEAD_FILE = os.path.join(BASE, "assets", "NotoSerifSC-Black.ttf")
        ...
        if unit["note"] == "cross_page":
            ...
            if len(parts) == len(blocks):
                for i, part in enumerate(parts):
                    mapping[blocks[i].split("+")[0]] = part.strip()
            else:
                print("警告：跨页单元 %s 的 [PB] 数量 %d 与页数 %d 不一致，整段落到第一块" % (
                    unit["unit_id"], len(parts) - 1, len(blocks)))
                mapping[primary0] = zh
        else:
            mapping[primary0] = zh
        for b in blocks:
            for extra in b.split("+")[1:]:
                mapping[extra] = ""
    return mapping


def continuation_block_ids(translated):
    """返回不属于翻译单元段首的块 ID（跨页单元中除第一块外的各页块），用于取消首行缩进。"""
    ids = set()
    for unit in translated["units"]:
        if unit["note"] != "cross_page":
            continue
...
def build_pages(structured, block_text, no_indent_ids=None):
    no_indent_ids = no_indent_ids or set()
    pages = []
            ...
            for b in p.get("blocks", []):
                ...
                if text:
                    items.append({
                        "kind": b["kind"],
                        "text": text,
                        "no_indent": b["block_id"] in no_indent_ids,
                    })
        pages.append({
            "page_no": p["page_no"],
...
def main():
    extracted = json.load(open(EXTRACTED, encoding="utf-8-sig"))
    structured = json.load(open(STRUCTURED, encoding="utf-8-sig"))
    translated = json.load(open(TRANSLATED, encoding="utf-8-sig"))

    size_by_page = {p["page_no"]: (p["page_size"][0], p["page_size"][1]) for p in extracted["pages"]}
    block_text = split_translated_units(translated)
    pages = build_pages(structured, block_text, continuation_block_ids(translated))

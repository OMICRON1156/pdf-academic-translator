...
def summarize_extracted(work):
    ...
    d = os.path.dirname(p)
    batch_files = sorted(f for f in os.listdir(d) if f.startswith("extracted_batch_") and f.endswith(".json"))
    if batch_files:
        lines.append("分批文件：%s" % ", ".join(batch_files))
...
def summarize_structured(work):
    ...
    if not os.path.isdir(d):
        return ["（结构化产物不存在）"]
    files = sorted(f for f in os.listdir(d) if f.startswith("structured_batch_") and f.endswith(".json"))
    if not files:
        return ["（结构化产物不存在）"]
...
def summarize_translated(work):
    ...
    files = sorted(f for f in os.listdir(d)
                   if f.startswith("translated_batch_") and f.endswith(".json")
                   and not f.endswith(".partial.json"))
    if not files:
        return ["（翻译产物不存在）"]
    ...
    for fn in files:
        ...
        n_bad = sum(1 for u in units if u.get("note") == "cross_page"
                    and u.get("zh_text", "").count("[PB]") != len(u.get("pages", [])) - 1)
        total += len(units)
        cross += n_cross
        pb_bad += n_bad
...
def summarize_render(work, name):
    ...
    epub = os.path.join(work, "05_rendered", name + "_zh.epub")
    if os.path.exists(epub):
        lines.append("【排版合成】成品 EPUB：%s" % epub)
